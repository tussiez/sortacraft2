...
const cacheControl = {
  addAll: function (cacheArr, storageName, throwOnFail) {
    if ("undefined" === typeof cacheArr) {
      throw new TypeError('Parameter "cacheArr" (argument 1) is not defined.');
    }
    if (!cacheArr.forEach) {
      throw new TypeError(
      ...
      if (Number(txt)) {
        ...
        if (ver > 0) {
          ...
          if (ver > current) {
            ...
            console.log(
              "Outdated game version detected, old: " + current + ".",
            );
            ...
            console.warn(
              "Cached game appears to be in a future state. Cached version: " +
                current,
            );
            //Time machine used
          }
