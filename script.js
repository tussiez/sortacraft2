...
worker.postMessage(["main", offscreen, window.innerWidth, window.innerHeight], [
  offscreen,
]);
...
worker.onmessage = function (e) {
  ...
  if (msg == "saveFile") {
    downloadFile(e.data[1]);
  }
};

function downloadFile(file) {
  let blob = window.URL.createObjectURL(
    new Blob([file], { type: "json/plain" }),
  );
  let ele = document.createElement("a");
  ele.setAttribute("href", blob);
  ele.setAttribute("download", "worldSave.sortacraft");
  ele.click();
}
...
window.load = function (dat) {
  worker.postMessage(["load", JSON.parse(dat)]);
};
