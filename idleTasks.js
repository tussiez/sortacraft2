...
Math.average = (arr)=>{
  if(!Array.isArray(arr)) throw new TypeError("Argument given is not an array.");
  let num = 0;
  arr.forEach((v)=>{
    num += v;
  });
  ...
  constructor(timesExceededLimit = 100) {
    ...
    this.takeSample = ()=>{
      return new Promise((res,rej)=>{
        ...
        const callback = ()=>{
          ...
          this.sample = [performance.measure(`PerfW#${token}`,`PerfW#${token}:start`,`PerfW#${token}:end`).duration];
    ...
    this.takeSampleAverage = (iter = 10)=>{
      return new Promise((res,rej)=>{
        ...
        const callback = ()=>{
          num++;
          performance.mark(`PerfW#${token}:end`);
          this.sample.push(performance.measure(`PerfW#${token}`,`PerfW#${token}:start`,`PerfW#${token}:end`).duration);
          performance.clearMarks(`PerfW#${token}`);
          if(num >= iter){
            res(this.sample);
            return;
    ...
    this.addEventListener = (name,callback)=>{
      ...
      if(!(name in listeners)){
        Object.defineProperty(listeners,name,{value:[callback],enumerable:true,configurable:true,writable:true});
      }
      else{
    ...
    this.getEventListeners = (name)=>{
      ...
      if(undefArgs > requiredArgs){
        throw new TypeError(`Failed to execute getEventListeners() on ${this.__proto__}, expected ${requiredArgs} arguments but got ${undefArgs} arguments instead.`);
      }
      if("string" !== typeof name){
        throw new TypeError("Argument 1 is not a string.");
      }
    ...
    this.removeEventListener = (name,callback)=>{
      ...
      if(undefArgs > requiredArgs){
        throw new TypeError(`Failed to execute removeEventListener() on ${this.__proto__}, expected ${requiredArgs} arguments but got ${undefArgs} arguments instead.`);
      }
      if("string" !== typeof name){
        throw new TypeError("Argument 1 is not a string.");
      }
      ...
      let indxOf = listeners[name].indexOf(callback);
      //If it doesn't exist
      if(~~indxOf){
        return 0;
      }
      else{
        let listener = listeners[name][indxOf];
    ...
    this.measurePerformance = ()=>{
      ...
      const iter = ()=>{
        ...
        let dur = performance.measure(`PerfW#${token}`,`PerfW#${token}:start`,`PerfW#${token}:end`).duration;
        performance.clearMarks(`PerfW#${token}`);
        if(avg.length < 100){
          avg.push(dur);
          if("window" in globalThis && "requestIdleCallback" in globalThis){
        ...
        if(timesExceeded < 0){
          ...
          fireListeners("performanceAlert",{"averageDuration":avgN,"duration":dur,"sample":Math.average(this.sample)});
        }
